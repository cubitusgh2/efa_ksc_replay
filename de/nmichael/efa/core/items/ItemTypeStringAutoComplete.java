 ...
 * and a standard Java 8 Swing popup (JPopupMenu + JList).
 *
 * Renderer: custom painter that highlights the first occurrence of the search string
 * with a yellow background (like the original request).
 *
 * The class no longer depends on project-specific AutoCompletePopupWindow.
 * 
...
public class ItemTypeStringAutoComplete extends ItemTypeString {
    ...
    // Standard Swing based autocomplete popup with painter renderer
    protected JPopupMenu autoCompletePopup;
    protected JList<String> acList;
    protected DefaultListModel<String> acModel;
    ...
    public void iniDisplay() {
        ...
        initAutoCompleteKeyListenersForField();
     ...
     * DOWN: select next item (including roundtrip)
     * UP: select prior item (including roundtrip)
     * PGDN: select item a page down and put it into view
     * PGUP: select item a page up and put it into view
     */
    private void initAutoCompleteKeyListenersForField() {
	 ...
	 // Key bindings on the text field to control acList while keeping focus in the text field
	    final JTextField tf = (JTextField) field;
	    javax.swing.InputMap im = tf.getInputMap(javax.swing.JComponent.WHEN_FOCUSED);
	    javax.swing.ActionMap am = tf.getActionMap();
	    
	    im.put(javax.swing.KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0), "acDown");
	    am.put("acDown", new javax.swing.AbstractAction() {
	        public void actionPerformed(java.awt.event.ActionEvent e) {
	            if (autoCompletePopup != null && autoCompletePopup.isVisible()) {
	                int idx = acList.getSelectedIndex();
	                if (idx < acModel.getSize() - 1) {
	                    acList.setSelectedIndex(idx + 1);
	                    acList.ensureIndexIsVisible(acList.getSelectedIndex());
	                }
	            } else {
	                showAutoCompletePopupForField(tf);
	            }
	        }
	    });
	
	    im.put(javax.swing.KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0), "acUp");
	    am.put("acUp", new javax.swing.AbstractAction() {
	        public void actionPerformed(java.awt.event.ActionEvent e) {
	            if (autoCompletePopup != null && autoCompletePopup.isVisible()) {
	                int idx = acList.getSelectedIndex();
	                if (idx > 0) {
	    ...
	    im.put(javax.swing.KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_DOWN, 0), "acPageDown");
	    am.put("acPageDown", new javax.swing.AbstractAction() {
	        public void actionPerformed(java.awt.event.ActionEvent e) {
	            if (autoCompletePopup != null && autoCompletePopup.isVisible()) {
	                int idx = acList.getSelectedIndex();
	                int next = Math.min(idx + 10, Math.max(0, acModel.getSize() - 1));
	                acList.setSelectedIndex(next);
	                acList.ensureIndexIsVisible(acList.getSelectedIndex());
	            } else {
	                showAutoCompletePopupForField(tf);
	            }
	    ...
	    im.put(javax.swing.KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_UP, 0), "acPageUp");
	    am.put("acPageUp", new javax.swing.AbstractAction() {
	        public void actionPerformed(java.awt.event.ActionEvent e) {
	            if (autoCompletePopup != null && autoCompletePopup.isVisible()) {
	                int idx = acList.getSelectedIndex();
	                int prev = Math.max(idx - 10, 0);
	                acList.setSelectedIndex(prev);
	                acList.ensureIndexIsVisible(acList.getSelectedIndex());
	            } else {
	                showAutoCompletePopupForField(tf);
	            }
	    ...
	    im.put(javax.swing.KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "acEnter");
	    am.put("acEnter", new javax.swing.AbstractAction() {
	        public void actionPerformed(java.awt.event.ActionEvent e) {
	            if (autoCompletePopup != null && autoCompletePopup.isVisible()) {
	                acceptSelectedFromPopup();
	            } else {
	                // Default: commit field (no-op here) - or call existing enter handling if needed
	            }
	        }
	    });
    }
    ...
    public void showOrRemoveAutoCompletePopupWindow() {
        if (useAutocompleteList) {
            ...
            if (f.isEnabled() && f.isEditable()) {
                if (!isAutoCompletePopupShowingForField(f)) {
                    hideAutoCompletePopup();
                    try {
                        Thread.sleep(50);
                    } catch(InterruptedException eignore) {}
                    showAutoCompletePopupForField(f);
                } else {
                    autoComplete(null);
                    hideAutoCompletePopup();
                }
    ...
    private void autoComplete(KeyEvent e) {
        ...
        if (e != null && e.getKeyCode() == -23) {
            return; // dieses Key-Event wurde von frueheren Implementierungen generiert
        }
        ...
        if (mode == Mode.normal
            ...
            if (e != null && e.getKeyCode() == KeyEvent.VK_DOWN) {
                if (withPopup && useAutocompleteList && isAutoCompletePopupShowingForField(field)) {
                ...
                if (e != null) {
                    complete = list.getFirst(prefix); // Taste gedrueckt --> OK, Wortanfang genuegt
                } else {
                    complete = list.getExact(field.getText().toLowerCase()); // keine Taste gedruekt --> nur richtig, wenn gesamtes Feld exakt vorhanden!
                }
            ...
            if (withPopup && useAutocompleteList && e != null && mode != Mode.none) {
                updateAutoCompletePopupContentAndSelect(complete != null ? complete : "");
                showAutoCompletePopupForField(field);
            }
        }
    ...
    private void handleFilteredList(KeyEvent e) {
        ...
        if (mode == Mode.normal || ((mode == Mode.enter || mode == Mode.escape || mode == Mode.none))) {
            ...
            if (e != null && ((e.getKeyCode() == KeyEvent.VK_DOWN)
                ...
                if ((e != null) && e.getKeyCode() != KeyEvent.VK_DOWN) {
                    if (Daten.efaConfig.getValuePopupContainsModeSelectPrefixItem()) {
                        ...
                        if (complete==null) {
                            complete = list.getFirst(textField.getText()); // Taste gedrueckt --> OK, Wortanfang genuegt
                        }
                    } else {
                        complete = list.getFirst(textField.getText()); // Taste gedrueckt --> OK, Wortanfang genuegt
                    }
            ...
            if (Logger.isTraceOn(Logger.TT_GUI, 5)) {
                ...
                Logger.log(Logger.DEBUG, Logger.MSG_GUI_DEBUGGUI, "AutoCompleteWindow showing at "+this.getName()+"="+isAutoCompletePopupShowingForField(textField));
            }
            ...
            if (e != null && (mode != Mode.normal && (
                    ...
                    isAutoCompletePopupShowingForField(textField))) {
                ...
                String sel = (acList != null ? acList.getSelectedValue() : null);
                complete = sel;
            ...
            if (withPopup && useAutocompleteList && e != null && mode != Mode.none && e.getKeyCode() != KeyEvent.VK_TAB) {
                updateAutoCompletePopupContentAndSelect(complete != null ? complete : "");
                showAutoCompletePopupForField(textField);
            }
        }

        if (mode == Mode.up) {
            if (withPopup && useAutocompleteList && isAutoCompletePopupShowingForField(textField)) {
                complete = list.getPrev(searchFor);
            } else {
                complete = list.getPrev(searchFor);
            }
       ...
       if (mode == Mode.delete) {
            if (withPopup && useAutocompleteList && e != null && mode != Mode.none) {
                ...
                updateAutoCompletePopupContentAndSelect(complete != null ? complete : textField.getText());
                showAutoCompletePopupForField(textField);
            }
        }
    ...
    public boolean isAutoCompleteWindowShowing() {
        if (field!=null) {
            return isAutoCompletePopupShowingForField((JTextField) field);
        }
        return false;
    ...
    // ----------------------------
    // Swing popup + custom painter renderer
    // ----------------------------
    ...
    private void initAutoCompletePopup() {
        ...
        acList.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        //Highlight search term in the items of the filtered list.
        acList.setCellRenderer(new PaintHighlightRenderer());

        // Mouse: click accepts selected entry
        acList.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (acList.getSelectedValue() != null) {
                    acceptSelectedFromPopup();
                }
            }
        });

        addComplexKeyListenerForPopup(acList);

        JScrollPane sp = new JScrollPane(acList);
        sp.setBorder(null);
        sp.setPreferredSize(new Dimension(300, 200)); // default, can be adapted
        autoCompletePopup.removeAll();
        autoCompletePopup.add(sp);
    }
    
    private void addComplexKeyListenerForPopup(JList<String> list) {
        // WICHTIG: Deaktiviere Focus-Traversal auf der JList
        // Dadurch wird TAB zur normalen Taste statt zum Focus-Manager-Signal
        list.setFocusTraversalKeysEnabled(false);
        
        // KeyListener mit keyPressed (wird VOR dem FocusManager aufgerufen)
        list.addKeyListener(new java.awt.event.KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                try {
                    // TAB / SHIFT+TAB: accept selected entry and close popup
                    ...
                    // LEFT / RIGHT: move caret inside text field
                    ...
                    // BACKSPACE / DELETE: edit text field at caret
                    if (code == KeyEvent.VK_BACK_SPACE) {
                        int pos = tf.getCaretPosition();
                        if (pos > 0) {
                            tf.getDocument().remove(pos - 1, 1);
                            tf.setCaretPosition(pos - 1);
                            updateAutoCompletePopupContent();
                            selectFirstMatch(tf.getText().trim().toLowerCase());
                        }
                        e.consume();
                        return;
                    }
                    ...
                    boolean isPrintable = (ch != KeyEvent.CHAR_UNDEFINED) && !e.isActionKey() && (e.getModifiersEx() & KeyEvent.CTRL_DOWN_MASK) == 0 && (e.getModifiersEx() & KeyEvent.ALT_DOWN_MASK) == 0;
                    if (isPrintable) {
                        int pos = tf.getCaretPosition();
                        tf.getDocument().insertString(pos, String.valueOf(ch), null);
                        tf.setCaretPosition(pos + 1);
                        updateAutoCompletePopupContent();
                        selectFirstMatch(tf.getText().trim().toLowerCase());
                        acList.requestFocusInWindow();
                        e.consume();
                        return;
                    }

                } catch (BadLocationException ex) {
                    Logger.log(Logger.DEBUG, Logger.MSG_GUI_DEBUGGUI, "BadLocationException in autocomplete key handler: " + ex.getMessage());
                }
            }
        });
    }
    ...
    /**
     * Selects first list entry that contains the filter (case-insensitive).
     * If none found, selects index 0 if available.
     */
    private void selectFirstMatch(String lowerFilter) {
        if (acModel == null || acModel.size() == 0) return;
        if (lowerFilter == null) lowerFilter = "";
        boolean selectByPrefix = Daten.efaConfig.getValuePopupContainsModeSelectPrefixItem();
        boolean searchByContains = Daten.efaConfig.getValuePopupContainsMode();
        
        int found = -1;
        for (int i = 0; i < acModel.size(); i++) {
            String s = acModel.get(i);
            if (selectByPrefix) {
				if (s != null && s.toLowerCase().startsWith(lowerFilter)) {
					found = i;
					break;
				}
			} else if (searchByContains==true) {
        ...
        final int idx = (found >= 0 ? found : (acModel.size() > 0 ? 0 : -1));
        if (idx >= 0) {
            acList.setSelectedIndex(idx);
            acList.ensureIndexIsVisible(idx);
        }
    }

    private void updateAutoCompletePopupContent() {
        if (acModel == null) return;
        acModel.clear();
        AutoCompleteList acl = getAutoCompleteList();
        if (acl == null) return;
        try { acl.update(); } catch (Throwable ignore) {}
        // try filter API if available
        try {
            String filterText = (field instanceof JTextField) ? ((JTextField) field).getText().trim() : "";
            acl.setFilterText(filterText);
        } catch (Throwable ignore) {}

        Vector<String> data = null;
        try {
            data = acl.getDataVisibleFiltered(); // preferred when available
        } catch (Throwable t1) {
            try {
                data = acl.getDataVisible();
            } catch (Throwable t2) {
                data = new Vector<>();
            }
        }
        for (String s : data) {
            acModel.addElement(s);
        }
    }

    private void updateAutoCompletePopupContentAndSelect(String selectValue) {
        updateAutoCompletePopupContent();
        if (selectValue != null && !selectValue.isEmpty() && acModel.size() > 0) {
            for (int i = 0; i < acModel.size(); i++) {
                if (acModel.get(i).equals(selectValue)) {
                    acList.setSelectedIndex(i);
                    acList.ensureIndexIsVisible(i);
                    return;
                }
            }
        } else if (acModel.size() > 0) {
            acList.setSelectedIndex(0);
        }
    }

    private void showAutoCompletePopupForField(JTextField f) {
        if (autoCompletePopup == null) initAutoCompletePopup();
        updateAutoCompletePopupContent();
        if (acModel.size() == 0) {
            hideAutoCompletePopup();
            return;
        }
        // ensure popup width at least field width
        Component comp = autoCompletePopup.getComponent(0);
        if (comp instanceof JScrollPane) {
            JScrollPane sp = (JScrollPane) comp;
            Dimension pref = sp.getPreferredSize();
            pref.width = Math.max(pref.width, f.getWidth());
            sp.setPreferredSize(pref);
        }
        autoCompletePopup.show(f, 2, f.getHeight());
        acList.requestFocusInWindow();
        // don't request focus in the list - keep focus in the text field
        if (acModel.size() > 0) acList.setSelectedIndex(0);
    }
    ...
    private boolean isAutoCompletePopupShowingForField(JTextField f) {
        return autoCompletePopup != null && autoCompletePopup.isVisible();
    }

    private void acceptSelectedFromPopup() {
        if (acList == null) return;
        String sel = acList.getSelectedValue();
        if (sel != null) {
            ((JTextField) field).setText(sel);
            // update status (button color, validation)
            autoComplete(null);
            hideAutoCompletePopup();
            JTextField f = (JTextField) field;
    ...
    private class PaintHighlightRenderer extends JComponent implements ListCellRenderer<String> {
        ...
        // Cached colors (sollten sich nicht ständig ändern)
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Draw background
            if (selectedFlag) {
                g2.setColor((selectionBackground != null ? selectionBackground : new JList().getBackground()));
            ...
            g2.setColor(selectedFlag ? (selectionForeground != null ? selectionForeground : this.getForeground()) : listForeground);
