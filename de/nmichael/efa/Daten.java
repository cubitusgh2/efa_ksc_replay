...
public class Daten {
	...
	public final static String VERSIONID = "2.5.3_#11c"; // VersionsID: Format: "X.Y.Z_MM"; final-Version z.B. 1.4.0_00;
														// beta-Version z.B. 1.4.0_#1  //# is not good, is used in efa.data.Waters 
	public final static String VERSIONRELEASEDATE = "01.09.2026"; // Release Date: TT.MM.JJJJ
	...
	public final static String EFA_JAVA_ARGUMENTS = "EFA_JAVA_ARGUMENTS"; // Environment Variable Name containing all
																			// arguments passed to the "java" command
	public static String efa_java_arguments = null; // Environment Variable Contents containing all arguments passed to
													// the "java" command
	...
	public final static String EFADOCUMENTATIONURL = "http://efa.nmichael.de/dokuwiki/doku.php";
	public final static String EFATRANSLATEWIKI = "http://kenai.com/projects/efa/pages/TranslatingEfa";
	public final static String EFAGITHUBURL = "https://github.com/nicmichael/efa/releases";
	public final static String EFACLASSICDOWNLOADURL = "http://efa.nmichael.de/download.html";
	public final static String EFAJAVAUPDATEURL = "http://efa.nmichael.de/dokuwiki/doku.php?id=quick_install#installation_von_java";
	...
	public static final String EFA_USERDATA_DIR = "efa2"; // <efauser> = ~/efa2/ Directory for efauser data (if not efa
															// program directory)
	...
	public static final String EFATYPESFILE = "types.cfg"; // <efauser>/cfg/types.cfg Konfiguration für EfaTypes
															// (Bezeichnungen)
	...
	public static final String EFA_SECFILE = "efa.sec"; // <efa>/program/efa.sec Hash von efa.jar: für Erstellen des
														// Admins
	...
	// public static String efaStyleDirectory = null; // Efa-Stylesheetverzeichnis,
	// mit "/" am Ende ("./fmt/layout/")
	...
	public static String pluginWebpage = "http://efa.nmichael.de/plugins.html"; // wird automatisch auf das in der o.g.
																				// Datei stehende gesetzt
	...
	public final static String INTERNET_EFAMAIL = "http://efa.rudern.de/efamail.pl"; // was:
																						// "http://cgi.snafu.de/nmichael/user-cgi-bin/efamail.pl";
	...
	public final static int AUTO_EXIT_MIN_RUNTIME = 60; // Minuten, die efa mindestens gelaufen sein muß, damit es zu
														// einem automatischen Beenden/Restart kommt (60)
	public final static int AUTO_EXIT_MIN_LAST_USED = 5; // Minuten, die efa mindestens nicht benutzt wurde, damit
															// Beenden/Neustart nicht verzögert wird (muß kleiner als
															// AUTO_EXIT_MIN_RUNTIME sein!!!) (5)
	public final static int WINDOWCLOSINGTIMEOUT = 600; // Timeout in Sekunden, nach denen im Direkt-Modus manche
	...
	public static void iniBase(int _applID) {
		...
		try {
			// ManagementFactory.getRuntimeMXBean().getName() == "12345@localhost" or
			// similar (not guaranteed by VM Spec!)
	...
	private static void iniMainDirectory() {
		...
		Daten.efaProgramDirectory = Daten.efaMainDirectory + "program" + Daten.fileSep; // just temporary, will be
																						// overwritten by
																						// iniDirectories()
		if (Logger.isTraceOn(Logger.TT_CORE, 9) || Logger.isDebugLoggingActivatedByCommandLine()) {
	...
	private static void iniEfaBaseConfig() {
		...
		if (!EfaUtil.canOpenFile(Daten.efaBaseConfig.getFileName())) {
			...
			if (!Daten.efaBaseConfig.writeFile()) {
				...
				String msg = International.getString("efa can't start") + ": " + LogString.fileCreationFailed(
						International.getString("Basic Configuration File"), Daten.efaBaseConfig.getFileName());
		...
		if (!Daten.efaBaseConfig.readFile()) {
			...
			String msg = International.getString("efa can't start") + ": " + LogString.fileOpenFailed(
					International.getString("Basic Configuration File"), Daten.efaBaseConfig.getFileName());
			...
			while (true) {
				...
				if (SimpleInputDialog.showInputDialog((Frame) null,
					...
					if (!efaBaseConfig.trySetUserDir(dir.getValue(), javaRestart)) {
						if (Logger.isTraceOn(Logger.TT_CORE, 9) || Logger.isDebugLoggingActivatedByCommandLine()) {
							Logger.log(Logger.DEBUG, Logger.MSG_CORE_STARTUPINITIALIZATION,
									"iniUserDirectory(): " + LogString.directoryNoWritePermission(dir.getValue(),
	...
	private static void iniLogging() {
		...
		Logger.log(Logger.INFO, Logger.MSG_EVT_EFASTART, International.getString("PROGRAMMSTART"));
		Logger.log(Logger.INFO, Logger.MSG_INFO_VERSION, "Version efa: " + Daten.VERSIONID + " -- Java: "
				+ Daten.javaVersion + " (JVM " + Daten.jvmVersion + ") -- OS: " + Daten.osName + " " + Daten.osVersion);

		if (Logger.isDebugLogging()) {
			Logger.log(Logger.INFO, Logger.MSG_LOGGER_DEBUGACTIVATED, "Debug Logging activated."); // do not
																									// internationalize!
		}
		...
		if (lastLogEntry != null && lastLogEntry.length() > 0
			...
			Logger.log(Logger.WARNING, Logger.MSG_WARN_PREVIOUSEXITIRREGULAR, International.getMessage(
					"efa wurde zuvor nicht korrekt beendet. Letzer Eintrag in Logdatei: {msg}", lastLogEntry));
		}
	}
	...
	private static void iniDirectories() {
		...
		// ./ausgabe
		// Daten.efaAusgabeDirectory = Daten.efaBaseConfig.efaUserDirectory + "fmt" +
		// Daten.fileSep;
		...
		// ./ausgabe/layout
		// Daten.efaStyleDirectory = Daten.efaAusgabeDirectory + "layout" +
		// Daten.fileSep;
	...
	public static void iniEfaSec() {
		 ...
		 * + "Um efa zu reaktivieren, wende Dich bitte an den Entwickler: ") +
		 * Daten.EMAILHELP; Logger.log(Logger.ERROR, Logger.MSG_CORE_EFASECCORRUPTED,
		 * msg); if (isGuiAppl()) { Dialog.error(msg); } haltProgram(HALT_EFASEC); }
		 */
	}

	// returns true if we need to create a new super admin (and are allowed to do
	// so)
	// returns false if we have a super admin and don't need to create one
	// halts efa if there is no super admin, but we're not allowed to create one
	// either
	public static boolean iniAdmins() {
		...
		try {
			...
			if (!isGuiAppl()) {
				...
				Logger.log(Logger.ERROR, Logger.MSG_CORE_ADMINSFAILEDOPEN, LogString.fileOpenFailed(
						((DataFile) Daten.admins.data()).getFilename(), International.getString("Administratoren")));
		...
		// we do have a admin file already that we can open. now check whether there's a
		// super admin configured as well
	...
	public static void iniEfaTypes(CustSettings custSettings) {
		...
		if (efaPreviousVersionID != null && (Daten.VERSIONID.compareTo(efaPreviousVersionID) > 0
				&& "2.2.2".compareTo(efaPreviousVersionID) > 0)) {
			StringBuilder changes = new StringBuilder();
			if (efaTypes.isConfigured(CATEGORY_SESSION, TYPE_SESSION_TRAINING)) {
				changes.append((changes.length() > 0 ? ", " : "") + TYPE_SESSION_TRAINING + "("
			...
			if (efaTypes.isConfigured(CATEGORY_SESSION, TYPE_SESSION_JUMREGATTA)) {
				changes.append((changes.length() > 0 ? ", " : "") + TYPE_SESSION_JUMREGATTA + "("
			...
			if (efaTypes.isConfigured(CATEGORY_SESSION, TYPE_SESSION_INSTRUCTION)) {
				changes.append((changes.length() > 0 ? ", " : "") + TYPE_SESSION_INSTRUCTION + "("
			...
			if (efaTypes.isConfigured(CATEGORY_SESSION, TYPE_SESSION_TOUR)) {
				changes.append((changes.length() > 0 ? ", " : "") + TYPE_SESSION_TOUR + "("
			...
			if (changes.length() > 0) {
				Logger.log(Logger.INFO, Logger.MSG_CORE_EFATYPESUPDATED,
						International.getMessage("Es wird empfohlen, die Fahrtarten {types} nicht mehr "
								...
								+ "können sie manuell wieder hinzugefügt werden. Weitere Hinweise auf http://efa.nmichael.de/help/fahrtarten.html",
								changes.toString()),
						true);
			}
		}
	...
	public static void iniScreenSize() {
		...
		Dialog.initializeScreenSize(null);
	}
	...
	public static void iniGUI() {
		...
		if (Daten.efaConfig != null) { // is null for applDRV
			try {
				...
				try {
					...
					Logger.log(Logger.ERROR, Logger.MSG_WARN_CANTSETLOOKANDFEEL,
							International.getString("Konnte Flatlaf nicht initialisieren:") + " " + e.getMessage());
				...
				if (Daten.efaConfig.getValueLookAndFeel().length() == 0) {
					...
					if (Daten.efaConfig.getValueLookAndFeel().endsWith(Daten.LAF_EFAFLAT_LIGHT)
							&& flatLafInitializationOK) {
						try {
							UIManager.setLookAndFeel(Daten.efaConfig.getValueLookAndFeel());
						} catch (Exception e) {
							flatLafInitializationOK = false;
							Logger.log(Logger.ERROR, Logger.MSG_WARN_CANTSETLOOKANDFEEL,
									International.getString("Konnte Flatlaf nicht initialisieren") + " "
											+ e.getMessage());
							Logger.logdebug(e);
						}
					} else if (Daten.efaConfig.getValueLookAndFeel().endsWith(Daten.LAF_EFAFLAT_LIGHT)
							&& flatLafInitializationOK == false) {
		...
		try {
			...
			if (efaConfig.getToolTipSpecialColors()) {
				Dialog.getUiDefaults().put("ToolTip.background",
						new ColorUIResource(efaConfig.getToolTipBackgroundColor()));
				Dialog.getUiDefaults().put("ToolTip.foreground",
						new ColorUIResource(efaConfig.getToolTipForegroundColor()));
			}

			if (!lookAndFeel.endsWith(Daten.LAF_METAL)) {
				// to make PopupMenu's work properly and not swallow the next MousePressed
				// Event, see: http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6753637
		...
		if (applID == APPL_EFABH) {
			try {
				Dialog.setGlobalFontSize(Daten.efaConfig.getValueEfaDirekt_BthsFontName(),
						Daten.efaConfig.getValueEfaDirekt_BthsFontSize(),
						Daten.efaConfig.getValueEfaDirekt_BthsFontStyle());
				Dialog.setGlobalTableFontSize(Daten.efaConfig.getValueEfaDirekt_BthsFontName(),
						Daten.efaConfig.getValueEfaDirekt_BthsTableFontSize());
			} catch (Exception e) {
		...
		if (applID == APPL_EFABASE) {
			try {
				Dialog.setGlobalFontSize(Daten.efaConfig.getValue_OtherFontName(),
						Daten.efaConfig.getValue_OtherFontSize(), Daten.efaConfig.getValue_OtherFontStyle());
				Dialog.setGlobalTableFontSize(Daten.efaConfig.getValue_OtherFontName(),
						Daten.efaConfig.getValue_OtherTableFontSize());
			} catch (Exception e) {
				Logger.log(Logger.WARNING, Logger.MSG_WARN_CANTSETFONTSIZE,
						International.getString("Schriftgröße konnte nicht geändert werden") + ": " + e.toString());
			}
		...
		// Needs to be placed here for adequate font scalings. If it is installed
		// earlier in the code, some scalings are not right in flatlaf.
	...
	public static boolean isGuiAppl() {
		return (applID == APPL_EFABASE || applID == APPL_EFABH || applID == APPL_EMIL || applID == APPL_ELWIZ
				|| applID == APPL_EDDI || applID == APPL_DRV)
	...
	private static boolean checkAndCreateDirectory(String dir) {
		...
		if (!f.isDirectory()) {
			...
			if (result == true) {
				Logger.log(Logger.WARNING, Logger.MSG_CORE_SETUPDIRS, International.getMessage(
						"Verzeichnis '{directory}' konnte nicht gefunden werden und wurde neu erstellt.", dir));
			} else {
				Logger.log(Logger.ERROR, Logger.MSG_CORE_SETUPDIRS, International
						.getMessage("Verzeichnis '{directory}' konnte weder gefunden, noch neu erstellt werden.", dir));
			}
			return result;
		}
		return true;
	...
	private static boolean tryCopy(String source, String dest, boolean alwaysCopyWhenNewer) {
		...
		if (copy) {
			if (EfaUtil.copyFile(source, dest)) {
				Logger.log(Logger.INFO, Logger.MSG_CORE_SETUPFILES, International
						.getMessage("Datei '{file}' wurde aus der Vorlage {template} neu erstellt.", dest, source));
				return true;
			} else {
				Logger.log(Logger.ERROR, Logger.MSG_CORE_SETUPFILES, International.getMessage(
						"Datei '{file}' konnte nicht aus der Vorlage {template} neu erstellt werden.", dest, source));
				return false;
			}
		}
			...
			if (applID != APPL_EFABH || applMode == APPL_MODE_ADMIN) {
				try {
					...
					infos.add("host.interface="
							+ EfaUtil.getInterfaceInfo(NetworkInterface.getByInetAddress(InetAddress.getLocalHost())));
					...
					if (jarfile != null && jarfile.length() > 0 && new File(jarfile).isFile()) {
						...
						try (JarFile jar = new JarFile(jarfile)) {
							...
							while (_enum.hasMoreElements() && (o = _enum.nextElement()) != null) {
								infos.add("java.jar.content=" + o + ":" + (jar.getEntry(o.toString()) == null ? "null"
	...
	public static void checkEfaVersion(boolean interactive) {
		...
		/*
		 * // Bei 1 Jahr alten Versionen alle 90 Tage prüfen, ob eine neue Version
		 * vorliegt if (EfaUtil.getDateDiff(Daten.VERSIONRELEASEDATE,EfaUtil.
		 * getCurrentTimeStampDD_MM_YYYY()) > 365 &&
		 * (Daten.efaConfig.efaVersionLastCheck == null ||
		 * Daten.efaConfig.efaVersionLastCheck.length() == 0 ||
		 * EfaUtil.getDateDiff(Daten.efaConfig.efaVersionLastCheck,EfaUtil.
		 * getCurrentTimeStampDD_MM_YYYY()) > 90) ) { if
		 * (Dialog.yesNoDialog(InternationalXX.
		 * getString("Prüfen, ob neue efa-Version verfügbar"), InternationalXX.
		 ...
		 * Daten.efaConfig.efaVersionLastCheck =
	...
	public static void checkJavaVersion(boolean interactive) {
		 ...
		 * 
		 * TMJ tmj = EfaUtil.string2date(Daten.javaVersion,0,0,0); int version =
		 * tmj.tag*100 + tmj.monat*10 + tmj.jahr;
		 * 
		 * if (version < 140) { if
		 * (Dialog.yesNoDialog(InternationalXX.getString("Java-Version zu alt"),
		 * InternationalXX.
		 * getMessage("Die von Dir verwendete Java-Version {version} wird von efa "+
		 ...
		 * InternationalXX.
		 * getMessage("Die von Dir verwendete Java-Version {version} ist bereits relativ alt. "
		 * +
	...
	public static void checkRegister() {
		...
		if (promptForRegistration && Daten.INTERNET_EFAMAIL != null) {
			if (BrowserDialog
					.openInternalBrowser(null, Daten.EFA_SHORTNAME, "file:" + HtmlFactory.createRegister(), 850, 750)
					.endsWith(".pl")) {
	...
	/**
	 * Returns the Java version as an integer number representing the "official"
